function boxplot_N1_peak(ccep_clin, ccep_prop, myDataPath)
close all
sub_label = [extractBetween(ccep_clin.dataName,'sub-','/ses')];
figure1 = figure('Name',sub_label{:},'Position',[349,383,891,679]);         % This is for the scatter plot 
...
for mode_N1 = 1:size(mode,2)
    ...
    % preallocation
    new_mat = zeros( size(ERs_both,1),2);   
    ...
    %                 
            ...
            boxplot([new_mat(:,1),new_mat(:,2)],'Labels',{'SPES-clin','SPES-prop'})
            title(sprintf('%s, Clinical SPES, %s',sub_label{1},mode{mode_N1}))
