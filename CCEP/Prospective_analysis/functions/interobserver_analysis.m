...
for subj = 1: size(uni_sublabel,2)  
    ...
    fun = @(s)~cellfun('isempty',strfind({files.name}',s));
...
%             if ~isfield(rater1.ccep,'mean_N1_lat') || ~isfield(rater2.ccep,'mean_N1_lat')
                % Than run this step 
%                  dataBase(subj,task) = select_n1_latency(rater1, rater2, myDataPath, uni_sublabel{subj},uni_tasklabel{task}, subj);
