...
names = fullfile(file10.folder, file10.name);
ccep = struct;
ccep.10stim= load(names);
ccep_all(i) = ccep;
%         ccep_allPat(i).ccep = ccep.ccep;
...
LocOnes = find_ones(dataBaseallstim, agreement.agreement_run);
...
plot_all_ccep_and_av(dataBaseallstim, dataBase2stim, myDataPath, LocOnes, agreement);
toc;
...
close all;
agreement_parameter = agreement_parameters(agreement, dataBaseallstim, dataBase2stim, myDataPath);
